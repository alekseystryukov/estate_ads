...
class RegistrationManager(models.Manager):
        ...
        if SHA1_RE.search(activation_key):
            ...
            if not profile.activation_key_expired():
                ...
                if not request.user.is_authenticated():
                    user = authenticate(username=user.email, password=None, anonymus=True)
                    login(request, user)
                user.is_active = True
                user.save()
    ...
    def create_inactive_user(self, email, password, site, send_email=True):
        ...
        """
        try:  # user can be already created for ads posting
            new_user = get_user_model().objects.get(email__iexact=email)
